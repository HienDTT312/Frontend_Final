...
function ProductDetail() {
  ...
  async function getProductComment() {
    let res = await axiosClient.get(
      `http://localhost:3001/service1/comment/${productId}`
    );
  ...
  const onClickDownload = (docId) => {
    console.log(`http://localhost:3001/service1/download/${docId}`);
    axiosClient
      .get(`http://localhost:3001/service1/download/${docId}`, { responseType: "blob"})
      .then((res) => {
  ...
  const handleSubmitComment = (comment, statusSwitch) => {
    axiosClient
      .post(`http://localhost:3001/service1/comment`, {
  ...
  const handleClickDeleteComment = (commentId, commentContent) => {
    axiosClient
      .delete(`http://localhost:3001/service1/comment/${commentId}`, {
  ...
  async function vote(action) {
    const res = await axiosClient.post("http://localhost:3001/service1/vote", {
      product_id: product.product_id,
      vote: action,
    });
    console.log(res.data);
  ...
  const handleClickDownloadAll = () => {
    axiosClient
      .get(`http://localhost:3001/service1/download-all/${product.product_id}`, { responseType: "blob"})
      .then((res) => {
