...
function ProductForm({ mode }) {
  ...
  const handleDeleteFile = (indexItem, docId) => {
    ...
    if (docId) {
      axiosClient
        .delete(`http://localhost:3001/service1/document/${docId}`)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (agree === false) {
      return alert("You have to agree with Brands & Condition");
    }
    ...
    if (mode === "create") {
      axiosClient
        .post(`http://localhost:3001/service1/product`, formData)
        .then((res) => navigate("/products/view", { replace: true }))
        .catch((err) => console.log(err));
    } else {
      formData.append("product_id", product.product_id);
      axiosClient
        .put(`http://localhost:3001/service1/product/${product.product_id}`, formData)
        .then((res) => navigate("/products/view", { replace: true }))
        .catch((err) => console.log(err));
    }
  };
